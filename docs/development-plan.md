...
- **Step 12-6**: `roof.kind` に `"gable"` を追加し、axis-aligned 矩形 +
  ridge_axis / ridge_height_mm で勾配屋根 (切妻) を作る。
  - `building/assemble.py` で `_validate_roof` を kind 別に分岐させ、
    `_assemble_roof` を `_assemble_flat_roof` / `_assemble_gable_roof` に
    分割。gable は shapely を経由せず 6 頂点 8 面の三角柱を numpy で
    手組み (追加依存なし)
  - `polygon` は 4 隅の axis-aligned 矩形に限定。点の順序は問わない。任意
    ポリゴンの gable (L 字平面など) と hip / 寄棟は Step 12-7+
  - サンプル: `samples/building_with_gable_roof.json` (80×60 mm の建物に
    棟方向 `x` / 棟高 8 mm の切妻屋根)
  - 依存追加なし
  - **やらないこと**: hip / 寄棟・mansard 等の他の屋根形状・eaves overhang
    (軒の出)・任意ポリゴンの gable (L 字・凹形)・複数棟線・屋根と壁の
    boolean union・屋根の色 / 材質メタデータ・棟の自動軸推定 (bbox 長辺
    方向を自動で選ぶ等)・Streamlit UI 露出
  - **完了条件**: `python -m meshforge convert
    --config samples/building_with_gable_roof.json out.stl` で壁 4 本の上に
    切妻屋根が乗った watertight STL が出る。既存
    `samples/building_minimal.json` / `samples/building_with_floor.json` /
...
- **Step 12-7**: `roof.kind` に `"hip"` (寄棟) を追加。gable と同じ
  axis-aligned 矩形 + `ridge_axis` + `ridge_height_mm` を受け、棟線の両端を
  bbox 内側に短辺の半分ぶん引き込む。
  - `building/assemble.py` で gable の検証ロジックを hip と共有 (rect +
    axis + ridge_height_mm) しつつ、hip だけ「`ridge_axis` が bbox の長辺と
    厳密一致」を追加要求。`_assemble_hip_roof` を gable と同じ 6 頂点 8 面
    トポロジで手組み (依存追加なし)
  - 正方形 footprint (W==D の pyramidal 縮退) は Step 12-8+ で `"pyramidal"`
    kind として別扱い
  - サンプル: `samples/building_with_hip_roof.json` (80×60 mm の建物 +
    `ridge_axis="x"` / `ridge_height_mm=8`)
  - 依存追加なし
  - **やらないこと**: pyramidal (W==D)・mansard・eaves overhang・任意ポリゴンの
    hip / gable・複数棟線・屋根と壁の boolean union・棟軸の自動推定・
    屋根の色 / 材質メタデータ・Streamlit UI 露出
  - **完了条件**: `python -m meshforge convert
    --config samples/building_with_hip_roof.json out.stl` で壁 4 本の上に
    寄棟屋根が乗った watertight STL が出る。既存
    `samples/building_minimal.json` / `samples/building_with_floor.json` /
    `samples/building_with_door.json` / `samples/building_with_roof.json` /
    `samples/building_with_gable_roof.json` / `samples/dome.png` の md5 は
...
- **Step 12-12**: 抽出 walls[] の axis-aligned 線分マージ。Canny が壁の両 edge
  ...
  - cluster 追加判定は「直交方向の近さ + **軸方向の overlap or gap <=
    --merge-gap-mm**」。これでドア開口で分断された壁や同じ y 上の別部屋の壁が
    1 本に統合されない。`--merge-gap-mm 0` を指定すれば overlap 必須
  - 斜め線分は merge せずそのまま残す (任意角度の merge は Step 12-13+)
  - `extract_walls()` に `merge=True` (デフォルト) / `merge_distance_mm=2.0` /
    `merge_angle_deg=5.0` / `merge_gap_mm=2.0` 引数を追加
  ...
  - **やらないこと**: 任意角度・斜め線の merge・複数 cluster をまたぐ merge
    (greedy first-match のみ)・rooms / openings / roof / furniture の自動抽出・
    Claude API 意味付け・Streamlit UI 露出
  - **完了条件**: `meshforge extract-walls samples/floor_plan_simple.png
    --pixel-mm 0.5 --wall-thickness-mm 4.0 --wall-height-mm 24.0
    --min-length-mm 30.0 -o out.json` で walls 数が **5** (Step 12-11 までは 10) に
    減る。`--no-merge` を渡すと 10 のまま (Step 12-11 互換)。gap で分断された
    線分は `--merge-gap-mm` 以下なら統合、超えれば別 wall として残す。merged
    JSON を convert に流すと watertight=True (verts=40 faces=60)。既存 9 サンプル
    (building_minimal / floor / door / roof / gable / hip / pyramidal /
    furniture / dome.png) の md5 は不変
