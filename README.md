...
- **画像から立体化 (Heightmap) タブ**: 画像や PDF をアップロード → 設定を選ぶ →
  「変換」ボタンで STL を作成。その場で 3D プレビューを回して確認し、
  ダウンロードできます。白黒画像の明るさを高さに変える、いちばん手軽な方法です。
- **間取り図から建物 (Building) タブ**: 建物の平面図から壁を自動で読み取って
  立体にする、より高度なタブです。読み取った壁を表で編集することもできます。

### 画面イメージ

#### 画像から立体化 (Heightmap) タブ

![meshforge heightmap タブ](docs/screenshots/heightmap-tab.png)

> 白黒画像（PNG / JPEG / PDF）をアップロードして、プリセットか手動で設定を
> 調整し「変換」を押すだけ。ロゴ・地形・平面図の凹凸モデル向けの、いちばん
> シンプルな流れです。

#### 間取り図から建物 (Building) タブ

![meshforge building タブ](docs/screenshots/editor.png)

> 「画像から抽出」を選んだ画面です。建物の平面図から壁を自動で読み取り、
> 立体にします。読み取った壁は表で編集できます。

> 「抽出して生成」を押すと、読み取った壁の中心線が赤、自動で見つけた部屋の
...
### 三角形が多すぎてスライサーに読み込めないとき

高解像度の写真などをそのまま変換すると、1 ピクセルあたり約 4 個の三角形が
できるため STL の三角形数が数百万〜数千万に膨らみ、Bambu Studio などが
「三角形が多すぎる」と読み込みを拒否することがあります。`convert` は既定で
**約 100 万三角形** を上限にハイトマップを自動縮小します（印刷時の物理サイズは
そのまま）。上限は `--max-triangles` で調整でき、`0` で無制限になります。

```sh
# 上限を 50 万に下げる（より軽く・確実に印刷したいとき）
.venv/bin/python -m meshforge convert samples/photo.png samples/photo.stl \
    --max-triangles 500000
```

ブラウザ UI でも「最大三角形数」で同じ調整ができます。
