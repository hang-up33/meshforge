...
#### Step 12-14: Extract 結果の line overlay

![extract walls overlay](docs/screenshots/overlay-preview.png)

> Step 12-14 で追加した overlay。`Extract & Build` 押下後、入力画像の上に
> 検出された `walls[]` の中心線が赤で重なって表示される。スクショは
> `samples/floor_plan_simple.png` を pixel_mm=0.5 / wall_thickness_mm=4 /
> wall_height_mm=24 / min_length_mm=30 で抽出した結果 (外周 4 + 内壁 1 =
> 5 本) を `_render_extract_overlay` と同じロジックでオフライン再現したもの。
> 公開デモではブラウザ上で直接見られる。
